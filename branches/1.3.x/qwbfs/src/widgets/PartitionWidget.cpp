...
AbstractFileSystem* PartitionWidget::entriesModel() const
{
	return lvDiscs->model();
}

AbstractFileSystem* PartitionWidget::importModel() const
{
...
{
	/*retranslateUi( this );
	lInformations->setText( tr( "%1 disc(s) on the partition - %2 disc(s) to import." ).arg( entriesModel()->rowCount() ).arg( importModel()->rowCount() ) );*/
}
